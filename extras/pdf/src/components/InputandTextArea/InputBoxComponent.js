...
function InputBoxComponent({
  ...
  const { timeStamp } = useContext(ChatContext)
  ...
  const submitAPI = (apiSub, text, setTextResponse, isOn) => {
    if (text) {
      ...
      var authOptions = {
        ...
        data: JSON.stringify({ prompt: text, filteredData: filteredData,sessionID:timeStamp}),
        headers: {
      ...
      <Accordion.Body>
        ...
        <Row>
          ...
          <Col md={showPreviewcol ? 12 : 6} sm={12} className="position-relative">
            ...
            />
            <img src={send} alt='' className="submit_icon position-absolute" onClick={() =>
              submitAPI(
                scope,
