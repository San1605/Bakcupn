...
import BNM_LOGO from "../../assets/bnmNamedLogo.png";
...
const NavBar = ({ handleHamburgerClick, hamburgerRef, hideSidebar }) => {
  ...
  return (
    <nav className="navbar bg-white flex items-center justify-between px-6 py-3 h-[60px]">
      <div className="sm:hidden flex items-center gap-4">
        <button onClick={handleHamburgerClick} disabled={!hideSidebar}>
          <img className="h-[15px] cursor-pointer" src={HAMBURGER} alt="" />
        </button>
        <div className="logo sm:h-full sm:hidden inline-blobk h-[33px] max-w-[121px] w-fit">
          <img
            ref={hamburgerRef}
      ...
      <div className="hidden sm:inline-block logo sm:h-full h-[40px] max-w-[121px] w-fit">
        <img
          className="h-full w-full cursor-pointer"
          src={BNM_LOGO}
          alt=""
