...
import AddCourseviewer from "../../../components/rolemanagementModals/addModals/AddCourseviewer";
import RemoveCourseViewer from "../../../components/rolemanagementModals/removeModal/RemoveCourseViewer";
import EditCourseViewer from "../../../components/rolemanagementModals/editModals/EditCourseViewer";
import { Bars } from "react-loader-spinner";

function CourseViewer() {
  const [eventkeyArr, setEventkeyArr] = useState([]);
  const {lpnamereleventfilter, lpnamesapi, listCourseViewer, courseviewlist, deletecourseViewer, loading} = useContext(GlobalContext);
  ...
  const onChangeEventhandler = (event) => {
    ...
    listCourseViewer(selectedpageno, courseSearchKey);
  };

  useEffect(() => {
    if (courseviewlist) {
      setPageCount(courseviewlist?.pages?.Total_Pages);
      setSelectedCourses(courseviewlist?.list);
    }
  }, [courseviewlist]);
  ...
  useEffect(()=>{
    listCourseViewer(currentPage, courseSearchKey, learningPath);
  },[learningPath])
  const searchinit = () => {
    if (courseSearchKey === "") {
      setSelectedCourses(courseviewlist?.list);
  ...
  return (
    <div className="hrbuddyContainer">
      ...
      <div className="hrbuddyListContainer bg-white">
        ...
        >
          Course Viewer List
          <span className="hrbuddyTotalCount ms-2">{courseviewlist ? courseviewlist.count : 0}</span>
        </div>
