...
function ComversionCommon() {
  ...
  useEffect(() => {
    setPageCount(
      Math.ceil(hrbuddyConversionList?.pages?.Total_Pages)
    );
  ...
  useEffect(() => {
    if (convarr.length > 0 && comdeplist.length > 0) {
      if( Array.isArray(hrbuddyConversionList) || currentPage !== comconprev.currentPage || hrBuddyName !== comconprev.hrBuddyName )
      {
      dispatch({
        type: "COM_CON_UPDATE",
        payload: {
