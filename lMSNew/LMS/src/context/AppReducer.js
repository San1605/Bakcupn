...
const AppReducer = (state, action) => {
  switch (action.type) {
    ...
    case "FILTER_BUDDY_LIST":
      return {
        ...state,
        buddylists: { ...state.buddylists, hrarr: action.payload },
        hrbuddylongerintegration: action.payload.length === 50,
        hrbuddypageno: 2
      }
    case "BUDDY_LIST":
      return {
        ...state,
        buddylists: { ...state.buddylists, hrarr: [...state.buddylists.hrarr, ...action.payload] },
        hrbuddylongerintegration: action.payload.length === 50,
        hrbuddypageno: state.hrbuddypageno + 1
      }
    ...
    case "FILTER_ADMIN_EMP_RES":
      return {
        ...state,
        dashlist: action.payload,
        adminemplongerintegration: action.payload.length === 50,
        adminempcurrentpage: 2
      }
    case "DASH_LIST":
      return {
        ...state,
        dashlist: [...state.dashlist, ...action.payload],
        adminemplongerintegration: action.payload.length === 50,
        adminempcurrentpage: state.adminempcurrentpage + 1
      }
    ...
    case "DOWNREPORTALL":
      return {
        ...
        downloadreportalldata: { ...state.downloadreportalldata,
           ...
           alist: action.payload.downrep1.map((item)=>{


            const startdate = item.Project_Start_Date.includes(",") ? item.Project_Start_Date.split(",").map((elem) => { return moment(elem).format("DD/MM/YYYY") }).toString() : item.Project_Start_Date.length > 0 ? moment(item.Project_Start_Date).format("DD/MM/YYYY") : "";
          const enddate = item.Project_End_Date.includes(",") ? item.Project_End_Date.split(",").map((elem) => { return moment(elem).format("DD/MM/YYYY") }).toString() : item.Project_End_Date.length > 0 ? moment(item.Project_End_Date).format("DD/MM/YYYY") : "";

          return(
            {
              ...item, Project_Start_Date:startdate, Project_End_Date:enddate
            }
          )
           ...
           blist: action.payload.downrep2.map((item)=>{
    ...
    case "ADMIN_DATA_ANALYTICS":
      return {
        ...state,
        adminDataAnalytics: { ...state.adminDataAnalytics, ...action.payload }
      }
    case "LOADING_NAME_ADMIN_ANALYTICS_TRUE":
      return {
        ...state,
        adminnameloading: true
      }
    case "LOADING_NAME_ADMIN_ANALYTICS_FALSE":
      return {
        ...state,
    ...
    case "ACTIVE_ADMIN_DASHBOARD":
      return {
        ...state,
        activeAdminDashboard: { ...state.activeAdminDashboard, ...action.payload },
      }
    ...
    case "EMPLOYEE_FOR_HR":
      return {
        ...state,
        empsforHr: action.payload
      }
    ...
    case "FILTER_DEPRES":
      return {
        ...state,
        depheadlistdata: { ...state.depheadlistdata, deparr: action.payload },
        deplongerintegration: action.payload.length === 50,
        depheaddatapageno: 2
      }
    case "DEPRES_LIST":
      return {
        ...state,
        depheadlistdata: { ...state.depheadlistdata, deparr: [...state.depheadlistdata.deparr, ...action.payload] },
        deplongerintegration: action.payload.length === 50,
        depheaddatapageno: state.depheaddatapageno + 1
      }
    case "DEP_HEAD_LIST":
      return {
        ...state,
    ...
    case "DEP_LOADING_TRUE":
      return {
        ...state,
        deploading: true
      }
    case "DEP_LOADING_FALSE":
      return {
        ...state,
        deploading: false
      }
    case "HR_LOADING_TRUE":
      return {
        ...state,
        hrloading: true
      }
    case "HR_LOADING_FALSE":
      return {
        ...state,
        hrloading: false
      }
    ...
    case "MUL_HR_SIMPLE":
      return {
        ...state,
        downloadrepoforhrsimple: action.payload.map((item) => {
          const startdate = item.Project_Start_Date.includes(",") ? item.Project_Start_Date.split(",").map((elem) => { return moment(elem).format("DD/MM/YYYY") }).toString() : item.Project_Start_Date.length > 0 ? moment(item.Project_Start_Date).format("DD/MM/YYYY") : "";
          const enddate = item.Project_End_Date.includes(",") ? item.Project_End_Date.split(",").map((elem) => { return moment(elem).format("DD/MM/YYYY") }).toString() : item.Project_End_Date.length > 0 ? moment(item.Project_End_Date).format("DD/MM/YYYY") : "";
          ...
          return (
            {
              ...item, Project_Start_Date: startdate,
    ...
    case "REPORTDATA_ADMIN_EMP":
      return {
        ...state,
        downrepoforadminemp: action.payload.map((item) => {
          const startdate = item.Project_Start_Date.includes(",") ? item.Project_Start_Date.split(",").map((elem) => { return moment(elem).format("DD/MM/YYYY") }).toString() : item.Project_Start_Date.length > 0 ? moment(item.Project_Start_Date).format("DD/MM/YYYY") : "";
          const enddate = item.Project_End_Date.includes(",") ? item.Project_End_Date.split(",").map((elem) => { return moment(elem).format("DD/MM/YYYY") }).toString() : item.Project_End_Date.length > 0 ? moment(item.Project_End_Date).format("DD/MM/YYYY") : "";
    ...
    case "REPORTDATA_INTERVIEW_SCHEDULING":
      return {
        ...state,
        downintersched: action.payload
      }
    case "REPORTDATA_DEP_HEAD":
      return {
        ...state,
        downrepoindephead: action.payload.map((item) => {
          const startdate = item.Project_Start_Date.includes(",") ? item.Project_Start_Date.split(",").map((elem) => { return moment(elem).format("DD/MM/YYYY") }).toString() : item.Project_Start_Date.length > 0 ? moment(item.Project_Start_Date).format("DD/MM/YYYY") : "";
          const enddate = item.Project_End_Date.includes(",") ? item.Project_End_Date.split(",").map((elem) => { return moment(elem).format("DD/MM/YYYY") }).toString() : item.Project_End_Date.length > 0 ? moment(item.Project_End_Date).format("DD/MM/YYYY") : "";
          return (
            {
    ...
    case "NOTIFICATION_ARRAY":
      ...
      case "PIP_REQUESTS":
        return {
          ...state,
          pipRequestsList: action.payload,
        };
  

    case "ADMIN_DATA_ANALYTICS_ACTIVE_USERS":
      return {
        ...state,
        adminDataAnalyticsActiveUsers:action.payload
      }
