...
import "./notificationRejectModal.css";

function NotificationRejectModal(props) {
  const { updatecourserequest } = useContext(GlobalContext);
  ...
  const handlestatus = () => {
    if (reason !== "") {
      ...
      updatecourserequest(2, reason, props.mailid, props.lp, props.request, props.dateMentioned);
    } else {
      ...
      >
        ...
        <Modal.Body style={{ padding: "8px 16px 0px 16px" }}>
          <Form>
            ...
            >
              <Form.Label
                className="reject-modal-reason-heading"
              >
